...
int main(void){
  ...
  double aux, aux2, max;
  int xmax, ymax, linhas=0;
  FILE * fentrada = NULL, * script=NULL;
  script = fopen("script.gpl", "wt");
  fentrada = fopen("posicoes.dat", "r");
  if (fentrada == NULL || script == NULL){
    if(fentrada == NULL) printf("Arquivo não encontrado.\n");
    if(script == NULL) printf("Arquivo de script não pode ser criado\n");
    return 0;
  }

  while(fscanf(fentrada,"%lf %lf", &aux, &aux2) != EOF){ 
    if(max == 0) max = aux2;
    if(max<aux2) max = aux2;
  }
  rewind(fentrada);

  char c;

  for(c = getc(fentrada); c != EOF; c = getc(fentrada)){
    if(c == '\n') linhas++;
  }
  ...
  fclose(fentrada);
  fprintf(script, "set term gif animate delay 1\nset out 'projetil.gif'\nset style line 1 lc rgb 'red' lt 1 lw 2 pt 7 ps 1.5 \nset xr [0:%d]\nset yr [0:%d]\nset xl 'Posição em x[m]'\nset yl 'Posição em y[m]'\nset title 'Lançamento do Projétil'\nset mxtics 2\nset mytics 2\nset grid mxtics xtics mytics ytics ls 0\ndo for [i=1:%d]{\nplot 'posicoes.dat' every ::1::i w l ls 1 t 'Trajetória', 'posicoes.dat' every ::i::i w p ls 1 t 'Projétil'\n}\nquit\n", xmax, ymax, linhas);
  
  fclose(script);

  system("gnuplot -p -e \"load 'script.gpl'\"");
  system("rm posicoes.dat script.gpl");
  system("eog projetil.gif");
